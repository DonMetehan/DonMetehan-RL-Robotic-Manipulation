...
default_noise_scale = 0.00  # Set your default noise level
...
if os.path.exists(episode_log_path):
    ...
    if len(lines) > 1:
        noise_scale_converge = float(lines[1].strip())
    else:
        noise_scale_converge = default_noise_scale
else:
    start_episode = 1
    noise_scale_converge = default_noise_scale
...
if os.path.exists(best_values_file):
    ...
    required_keys = [
        ...
        "total_reward_converge_reward", "total_reward_converge_distance"
    ]
    ...
    base_theta_1_converge = data_best["best_theta_1_reward"] 
    base_theta_2_converge = data_best["best_theta_2_reward"] 

    # ✅ Total Reward converge değerlerini her metrik için ayrı al
    total_reward_converge_reward = data_best["total_reward_converge_reward"]
    total_reward_converge_distance = data_best["total_reward_converge_distance"]


    
    # ✅ Best Reward Data (En yüksek ödül bazlı path)
    best_reward_data = {
        'total_reward_converge': data_best["total_reward_converge_reward"],
    ...
    lowest_distance_data = {
        'total_reward_converge': data_best["total_reward_converge_distance"],
    ...
    if os.path.exists("model_converge.npz"):
        data_converge = np.load("model_converge.npz")  # ✅ Load the file first
        base_theta_1_converge = data_converge["theta_1_converge"]
        base_theta_2_converge = data_converge["theta_2_converge"]
        print("converge Thetas loaded from model_converge.npz")
    else:
        base_theta_1_converge = np.random.randn(7, 3)
        base_theta_2_converge = np.random.randn(7)
        print("No previous Thetas found. Initializing new ones.")

# ✅ Ensure best_theta is always defined before saving
best_theta_1_converge = base_theta_1_converge
best_theta_2_converge = base_theta_2_converge
...
for episode in range(start_episode, start_episode + n_episodes):
    ...
    print(f"Noise Level : {noise_scale_converge}")
    print("----------------------------------------------------------------------")
    paths=[]
    paths_converge = []
    
    rollout_converge_rewards = []
    ...
    for rollout in range(n_rollouts):
        ...
        noisy_theta_1_converge = base_theta_1_converge + noise_scale_converge * np.random.randn(7, 3)
        noisy_theta_2_converge = base_theta_2_converge + noise_scale_converge * np.random.randn(7)
                        
        path, path_converge, save_flag= Sample(env, noisy_theta_1_converge, noisy_theta_2_converge, T, rollout, plot=False)
        
        paths.append(path)
        paths_converge.append(path_converge)
        
        rollout_converge_rewards.append(path_converge['total_reward_converge'])
        rollout_distances.append(path['distances'])
        

        best_updated = False

        # ✅ Check and update best reward
        if path_converge['total_reward_converge'] > best_reward_ever:
            best_reward_ever = path_converge['total_reward_converge']
            best_reward_episode = episode
            best_reward_rollout = rollout + 1
            best_theta_1_converge = noisy_theta_1_converge
            best_theta_2_converge = noisy_theta_2_converge
            best_reward_data = path_converge.copy()  # ✅ This ensures the dictionary does not lose keys
        ...
        if path_converge['final_distance'] < lowest_distance_ever:
            lowest_distance_ever = path_converge['final_distance']
            lowest_distance_episode = episode
            lowest_distance_rollout = rollout + 1
            lowest_distance_data = path_converge.copy()  
            best_updated = True
        ...
        if best_updated:
            np.savez("best_values.npz",
                     ...
                     total_reward_converge_reward=best_reward_data["total_reward_converge"],
                     best_theta_1_reward=best_reward_data["theta_converge"]["theta_1"],
                     best_theta_2_reward=best_reward_data["theta_converge"]["theta_2"],
                     ...
                     total_reward_converge_distance=lowest_distance_data["total_reward_converge"],
                     best_theta_1_distance=lowest_distance_data["theta_converge"]["theta_1"],
                     best_theta_2_distance=lowest_distance_data["theta_converge"]["theta_2"],
                     best_final_distance_distance=lowest_distance_data["final_distance"])
            
            print("✅ Updated best values saved.")
        print(f"Rollout {rollout + 1} Converge Reward: {path_converge['total_reward_converge']}")
        if save_flag:
            plt.figure()
            plt.plot(range(len(path['distances'])), path['distances'], marker='o')
            plt.xlabel("Time")
            plt.ylabel("Distance")
            plt.title(f"Episode {episode} - Rollout {rollout + 1}  Distances")
            plt.grid(True)
            plt.savefig(os.path.join(distance_plot_episode_dir, f"Rollout {rollout + 1} Distances.png"))
            plt.close()
            ...
            if len(path["Velocity"]) > 0:
                ...
                plt.xlabel("Time")
                plt.ylabel("Velocity")
                plt.title(f"Episode {episode} - Rollout {rollout + 1} - Joint Velocities")
                plt.legend()
                plt.grid(True)
            ...
            if len(path["Torques"]) > 0:
                ...
                plt.xlabel("Time")
                plt.ylabel("Torque")
                plt.title(f"Episode {episode} - Rollout {rollout + 1} - Joint Torques")
                plt.legend()
                plt.grid(True)
                ...
                for i, frame in enumerate(path['frames']):
                    frame_path = os.path.join(frame_dir, f"converge_after{i}.png")
                    imageio.imwrite(frame_path, np.flipud(frame))
               
    noise_scale_converge = max(noise_scale_converge * noise_decay, 0.0000) 

    success_count = sum([p['is_successful_converge'] for p in paths_converge])
    ...
    with open(info_path, 'a') as f:
        ...
        f.write("\nConverge Phase:\n")
        elite_theta_1_converge, elite_theta_2_converge, elite_indices_converge = cem.select_elite_thetas_converge( paths_converge)
        theta_1_update_norm = np.linalg.norm(elite_theta_1_converge - base_theta_1_converge)
        theta_2_update_norm = np.linalg.norm(elite_theta_2_converge - base_theta_2_converge)
        
        f.write(f"Standard Deviation of Rewards: {np.std(rollout_converge_rewards):.2f}\n")
        f.write(f"Noise Scale: {noise_scale_converge:.5f}\n")
        f.write(f"Success Rate: {success_rate:.2%}\n")
        f.write(f"Theta Update Norm: {theta_1_update_norm:.5f}\n")
        f.write(f"Theta Update Norm: {theta_2_update_norm:.5f}\n")
        f.write("---------------------------------------\n")
        f.write("Elite Rollouts for Converge Phase:\n")
        for idx in elite_indices_converge:
            elite_path = paths_converge[idx]
            f.write(f"Rollout {idx + 1} - Total Reward: {elite_path['total_reward_converge']}\n")
            f.write(f"Final Distance of Gripper to Target: {elite_path['final_distance']}\n")
            f.write(f"Theta_1:\n{elite_path['theta_converge']['theta_1']}\n")
            f.write(f"Theta_2:\n{elite_path['theta_converge']['theta_2']}\n\n")
            
    
    print(f"Episode {episode} bilgileri kaydedildi: {info_path}")

    base_theta_1_converge, base_theta_2_converge = elite_theta_1_converge, elite_theta_2_converge

    np.savez("model_converge.npz", theta_1_converge = base_theta_1_converge, theta_2_converge = base_theta_2_converge)
    episode_num += 1
    # Update last saved episode number
    with open(episode_log_path, "w") as f:
        f.write(f"{episode}\n{noise_scale_converge}")
...
with open(best_log_path, 'w') as f:
    ...
    if best_reward_data:
        ...
        f.write(f"   - Total Reward (By Reward): {best_reward_data['total_reward_converge']}\n")
        f.write(f"   - Final Distance: {best_reward_data['final_distance']}\n")
        f.write(f"   - Theta_1:\n{best_reward_data['theta_converge']['theta_1']}\n")
        f.write(f"   - Theta_2:\n{best_reward_data['theta_converge']['theta_2']}\n\n")
    ...
    if lowest_distance_data:
        ...
        f.write(f"   - Total Reward (By Distance): {lowest_distance_data['total_reward_converge']}\n")
        f.write(f"   - Theta_1:\n{lowest_distance_data['theta_converge']['theta_1']}\n")
        f.write(f"   - Theta_2:\n{lowest_distance_data['theta_converge']['theta_2']}\n\n")
