...
default_noise_scale_lift = 0  # Set your default noise level
...
if os.path.exists(episode_log_path):
    ...
    if len(lines) > 1:
        noise_scale_lift = float(lines[1].strip())
    else:
        noise_scale_lift = default_noise_scale_lift
else:
    start_episode = 1
    noise_scale_lift = default_noise_scale_lift
...
data_push = np.load("model_push.npz")
base_theta_1_push = data_push["theta_1_push"]
base_theta_2_push = data_push["theta_2_push"]
print("Push parameters loaded from model_push.npz")
...
if os.path.exists(best_values_file):
    ...
    required_keys = [
        ...
        "total_reward_lift_reward", "total_reward_lift_distance"
    ]
    ...
    base_theta_1_lift = data_best["best_theta_1_reward"] 
    base_theta_2_lift = data_best["best_theta_2_reward"]  

    # ✅ Total Reward lift değerlerini her metrik için ayrı al
    total_reward_lift_reward = data_best["total_reward_lift_reward"]
    total_reward_lift_distance = data_best["total_reward_lift_distance"]


    
    # ✅ Best Reward Data (En yüksek ödül bazlı path)
    best_reward_data = {
        'total_reward_lift': data_best["total_reward_lift_reward"],
    ...
    lowest_distance_data = {
        'total_reward_lift': data_best["total_reward_lift_distance"],
    ...
    if os.path.exists("model_lift.npz"):
        data_lift = np.load("model_lift.npz")  # ✅ Load the file first
        base_theta_1_lift = data_lift["theta_1_lift"]
        base_theta_2_lift = data_lift["theta_2_lift"]
        print("lift Thetas loaded from model_lift.npz")
    else:
        base_theta_1_lift = np.random.randn(7, 3)
        base_theta_2_lift = np.random.randn(7)
        print("No previous Thetas found. Initializing new ones.")

# ✅ Ensure best_theta is always defined before saving
best_theta_1_lift = base_theta_1_lift
best_theta_2_lift = base_theta_2_lift
...
for episode in range(start_episode, start_episode + n_episodes):
    ...
    print(f"Noise Level : {noise_scale_lift}")
    print("----------------------------------------------------------------------")
    paths=[]
    paths_lift = []
    
    rollout_lift_rewards = []
    rollout_bread_to_lift_target = [] 
    ...
    for rollout in range(n_rollouts):
        ...
        noisy_theta_1_lift = base_theta_1_lift + noise_scale_lift * np.random.randn(7, 3)
        noisy_theta_2_lift = base_theta_2_lift + noise_scale_lift * np.random.randn(7)
        
        path, path_lift, save_flag = Sample(env, base_theta_1_converge, base_theta_2_converge,base_theta_1_push,base_theta_2_push, noisy_theta_1_lift, noisy_theta_2_lift,rollout, T)
        
        
        paths.append(path)
        paths_lift.append(path_lift)
        
        rollout_lift_rewards.append(path_lift['total_reward_lift'])
        rollout_bread_to_lift_target.append(path_lift['bread_to_lift_target'])
                
        for i, frame in enumerate(path['frames']):
            frame_path = os.path.join(frame_dir, f"lift_after_{i}.png")
        ...
        if path_lift['total_reward_lift'] > best_reward_ever:
            best_reward_ever = path_lift['total_reward_lift']
            best_reward_episode = episode
            best_reward_rollout = rollout + 1
            best_theta_1_lift = noisy_theta_1_lift
            best_theta_2_lift = noisy_theta_2_lift
            best_reward_data = path_lift.copy()  # ✅ This ensures the dictionary does not lose keys

            best_updated = True
            print("Highest reward achieved. Thetas and best reward saved.")

        # ✅ Check and update lowest distance
        if path_lift['final_distance'] < lowest_distance_ever:
            lowest_distance_ever = path_lift['final_distance']
            lowest_distance_episode = episode
            lowest_distance_rollout = rollout + 1
            lowest_distance_data = path_lift.copy()  
            best_updated = True
        ...
        if best_updated:
            np.savez("best_values.npz",
                     ...
                     total_reward_lift_reward=best_reward_data["total_reward_lift"],
                     best_theta_1_reward=best_reward_data["theta_lift"]["theta_1"],
                     best_theta_2_reward=best_reward_data["theta_lift"]["theta_2"],
                     ...
                     total_reward_lift_distance=lowest_distance_data["total_reward_lift"],
                     best_theta_1_distance=lowest_distance_data["theta_lift"]["theta_1"],
                     best_theta_2_distance=lowest_distance_data["theta_lift"]["theta_2"],
                     best_final_distance_distance=lowest_distance_data["final_distance"])        
            print("✅ Updated best values saved.")


        
        print(f"Episode {episode} - Rollout {rollout + 1} lift Reward: {path_lift['total_reward_lift']}")
        if save_flag:
            plt.figure()
            plt.plot(range(len(path_lift['bread_to_lift_target'])),path_lift['bread_to_lift_target'], marker='o')
            plt.xlabel("Time")
            plt.ylabel("Bread to lift Target Distance")
    ...
    success_count = sum([p['is_successful_lift'] for p in paths_lift])
    ...
    with open(info_path, 'a') as f:
        ...
        f.write("\nlift Phase:\n")
        elite_theta_1_lift, elite_theta_2_lift, elite_indices_lift = cem.select_elite_thetas_lift( paths_lift)
        theta_1_update_norm = np.linalg.norm(elite_theta_1_lift - base_theta_1_lift)
        theta_2_update_norm = np.linalg.norm(elite_theta_2_lift - base_theta_2_lift)
        
        f.write(f"Standard Deviation of Rewards: {np.std(rollout_lift_rewards):.2f}\n")
        f.write(f"Noise Scale: {noise_scale_lift:.5f}\n")
        f.write(f"Success Rate: {success_rate:.2%}\n")
        f.write(f"Theta Update Norm: {theta_1_update_norm:.5f}\n")
        f.write(f"Theta Update Norm: {theta_2_update_norm:.5f}\n")
        f.write("---------------------------------------\n")
        f.write("Elite Rollouts for lift Phase:\n")
        for idx in elite_indices_lift:
            elite_path = paths_lift[idx]
            f.write(f"Rollout {idx + 1} - Total Reward: {elite_path['total_reward_lift']}\n")
            f.write(f"Final Distance of Gripper to Target: {elite_path['final_distance']}\n")
            f.write(f"Theta_1:\n{elite_path['theta_lift']['theta_1']}\n")
            f.write(f"Theta_2:\n{elite_path['theta_lift']['theta_2']}\n\n")
            
    
    print(f"Episode {episode} bilgileri kaydedildi: {info_path}")
    noise_scale_lift *= noise_decay
    base_theta_1_lift, base_theta_2_lift = elite_theta_1_lift, elite_theta_2_lift

    np.savez("model_lift.npz", theta_1_lift = base_theta_1_lift, theta_2_lift = base_theta_2_lift)
...
with open(best_log_path, 'w') as f:
    ...
    if best_reward_data:
        ...
        f.write(f"   - Total Reward (By Reward): {best_reward_data['total_reward_lift']}\n")
        f.write(f"   - Final Distance: {best_reward_data['final_distance']}\n")
        f.write(f"   - Theta_1:\n{best_reward_data['theta_lift']['theta_1']}\n")
        f.write(f"   - Theta_2:\n{best_reward_data['theta_lift']['theta_2']}\n\n")
    ...
    if lowest_distance_data:
        ...
        f.write(f"   - Total Reward (By Distance): {lowest_distance_data['total_reward_lift']}\n")
        f.write(f"   - Theta_1:\n{lowest_distance_data['theta_lift']['theta_1']}\n")
        f.write(f"   - Theta_2:\n{lowest_distance_data['theta_lift']['theta_2']}\n\n")
