...
default_noise_scale_pre_lift = 0.0 # Set your default noise level
T = 120
n_episodes = 1
n_rollouts = 1
noise_decay = 0.97
...
if os.path.exists(episode_log_path):
    ...
    if len(lines) > 1:
        noise_scale_pre_lift = float(lines[1].strip())
    else:
        noise_scale_pre_lift = default_noise_scale_pre_lift
else:
    start_episode = 1
    noise_scale_pre_lift = default_noise_scale_pre_lift
...
if os.path.exists(best_values_file):
    ...
    base_theta_1_pre_lift = data_best["best_theta_1_reward"] 
    base_theta_2_pre_lift = data_best["best_theta_2_reward"]  

    # ✅ Total Reward pre_lift değerlerini her metrik için ayrı al
    total_reward_pre_lift_reward = data_best["total_reward_pre_lift_reward"]


    
    # ✅ Best Reward Data (En yüksek ödül bazlı path)
    best_reward_data = {
        'total_reward_pre_lift': data_best["total_reward_pre_lift_reward"],
        'theta_pre_lift': {
    ...
    if os.path.exists("model_pre_lift.npz"):
        data_pre_lift = np.load("model_pre_lift.npz")  # ✅ Load the file first
        base_theta_1_pre_lift = data_pre_lift["theta_1_pre_lift"]
        base_theta_2_pre_lift = data_pre_lift["theta_2_pre_lift"]
        print("pre_lift Thetas loaded from model_pre_lift.npz")
    else:
        base_theta_1_pre_lift = np.random.randn(7, 6)
        base_theta_2_pre_lift = np.random.randn(7)
        print("No previous Thetas found. Initializing new ones.")

# ✅ Ensure best_theta is always defined before saving
best_theta_1_pre_lift = base_theta_1_pre_lift
best_theta_2_pre_lift = base_theta_2_pre_lift
...
for episode in range(start_episode, start_episode + n_episodes):
    ...
    print(f"Noise Level : {noise_scale_pre_lift}")
    print("----------------------------------------------------------------------")
    paths=[]
    paths_pre_lift = []
    
    rollout_pre_lift_rewards = []
    ...
    for rollout in range(n_rollouts):
        ...
        noisy_theta_1_pre_lift = base_theta_1_pre_lift + noise_scale_pre_lift * np.random.randn(7, 6)
        noisy_theta_2_pre_lift = base_theta_2_pre_lift + noise_scale_pre_lift * np.random.randn(7)
        
        path, path_pre_lift, save_flag = Sample(env, base_theta_1_converge, base_theta_2_converge,base_theta_1_push,base_theta_2_push, noisy_theta_1_pre_lift, noisy_theta_2_pre_lift,rollout, T)
        
        
        paths.append(path)
        paths_pre_lift.append(path_pre_lift)
        
        rollout_pre_lift_rewards.append(path_pre_lift['total_reward_pre_lift'])
                    
        for i, frame in enumerate(path['frames']):
            frame_path = os.path.join(frame_dir, f"prelift{i}.png")
        ...
        if path_pre_lift['total_reward_pre_lift'] > best_reward_ever:
            best_reward_ever = path_pre_lift['total_reward_pre_lift']
            best_reward_episode = episode
            best_reward_rollout = rollout + 1
            best_theta_1_pre_lift = noisy_theta_1_pre_lift
            best_theta_2_pre_lift = noisy_theta_2_pre_lift
            best_reward_data = path_pre_lift.copy()  # ✅ This ensures the dictionary does not lose keys
        ...
        if best_updated:
            np.savez("best_values.npz",
                     ...
                     total_reward_pre_lift_reward=best_reward_data["total_reward_pre_lift"],
                     best_theta_1_reward=best_reward_data["theta_pre_lift"]["theta_1"],
                     best_theta_2_reward=best_reward_data["theta_pre_lift"]["theta_2"])        
            print("✅ Updated best values saved.")


        
        print(f"Episode {episode} - Rollout {rollout + 1} pre_lift Reward: {path_pre_lift['total_reward_pre_lift']}")
        if save_flag:
    ...
    success_count = sum([p['is_successful_pre_lift'] for p in paths_pre_lift])
    ...
    with open(info_path, 'a') as f:
        ...
        f.write("\npre_lift Phase:\n")
        elite_theta_1_pre_lift, elite_theta_2_pre_lift, elite_indices_pre_lift = cem.select_elite_thetas_pre_lift( paths_pre_lift)
        theta_1_update_norm = np.linalg.norm(elite_theta_1_pre_lift - base_theta_1_pre_lift)
        theta_2_update_norm = np.linalg.norm(elite_theta_2_pre_lift - base_theta_2_pre_lift)
        
        f.write(f"Standard Deviation of Rewards: {np.std(rollout_pre_lift_rewards):.2f}\n")
        f.write(f"Noise Scale: {noise_scale_pre_lift:.5f}\n")
        f.write(f"Success Rate: {success_rate:.2%}\n")
        f.write(f"Theta Update Norm: {theta_1_update_norm:.5f}\n")
        f.write(f"Theta Update Norm: {theta_2_update_norm:.5f}\n")
        f.write("---------------------------------------\n")
        f.write("Elite Rollouts for pre_lift Phase:\n")
        for idx in elite_indices_pre_lift:
            elite_path = paths_pre_lift[idx]
            f.write(f"Rollout {idx + 1} - Total Reward: {elite_path['total_reward_pre_lift']}\n")
            f.write(f"Theta_1:\n{elite_path['theta_pre_lift']['theta_1']}\n")
            f.write(f"Theta_2:\n{elite_path['theta_pre_lift']['theta_2']}\n\n")
            
    
    print(f"Episode {episode} bilgileri kaydedildi: {info_path}")
    noise_scale_pre_lift *= noise_decay
    base_theta_1_pre_lift, base_theta_2_pre_lift = elite_theta_1_pre_lift, elite_theta_2_pre_lift

    np.savez("model_pre_lift.npz", theta_1_pre_lift = base_theta_1_pre_lift, theta_2_pre_lift = base_theta_2_pre_lift)
    # Update last saved episode number
    with open(episode_log_path, "w") as f:
        f.write(f"{episode}\n{noise_scale_pre_lift}")
...
with open(best_log_path, 'w') as f:
    ...
    if best_reward_data:
        ...
        f.write(f"   - Total Reward (By Reward): {best_reward_data['total_reward_pre_lift']}\n")
        f.write(f"   - Theta_1:\n{best_reward_data['theta_pre_lift']['theta_1']}\n")
        f.write(f"   - Theta_2:\n{best_reward_data['theta_pre_lift']['theta_2']}\n\n")
